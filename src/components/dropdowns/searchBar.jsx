...
import '../../images/bby-stars.png';
import '../../styles/searchStars.scss';

const axios = require('axios');

let apiServer;
(process.env.NODE_ENV === 'development') ? apiServer = 'http://localhost:3000/api' : apiServer = 'https://api.westbuy.org/api';
...
class SearchBar extends Component {
  ...
  getProducts (productName) {
    return axios.post(apiServer + '/products/search', { name: productName });
  }
  
  getRating (id) {
    axios.post(apiServer + '/reviews/stats', { productId: id })
      .then(result => {
  ...
  getRatings () {
    ...
    let ids = searchListData.map(item => {
      return item.ID
    });
    ...
    return (
      <div className="col m-2">
        { this.state.searchListData.map((item, index) => {
          return <a href={ '../' + item.ID } className="search-item row mr-1"
                    key={ `search-item-${ index }` }>{ item.Name }</a>
        }) }
      </div>
    )
  }
